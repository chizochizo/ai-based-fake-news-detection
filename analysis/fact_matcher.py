...
def extract_fact_structure(text):
    ...
    # NUMBERS
    # ==========================================
    numbers = re.findall(r"\b\d+(?:st|nd|rd|th)?\b", text.lower())
    ...
    structure["noun_chunks"] = list(set(noun_chunks))
...
def has_entity_overlap(claim_entities, evidence_entities):
    for claim_ent in claim_entities:
        for evidence_ent in evidence_entities:
            ...
            wikidata_score = entity_overlap_score(
                claim_ent.get("wikidata_ids", []),
                evidence_ent.get("wikidata_ids", [])
            )
            if wikidata_score > 0:
                return True
            ...
            similarity = fuzz.token_sort_ratio(
                claim_ent["text"], evidence_ent["text"])
            if similarity >= 80:
                return True
    return False
...
def detect_contextual_conflicts(claim_struct, evidence_struct):
    conflicts = []
    claim_recent = bool(claim_struct["recency_terms"])
    evidence_old = bool(evidence_struct["old_terms"])
    evidence_years = evidence_struct["years"]
...
def detect_conflicts(claim_struct, evidence_struct):
    ...
    entity_related = has_entity_overlap(
        claim_struct["entities"], evidence_struct["entities"])

    # ==========================================
    # NUMBER CONFLICTS
    # ==========================================
    claim_numbers = set(claim_struct["numbers"])
    evidence_numbers = set(evidence_struct["numbers"])
    shared_numbers = claim_numbers & evidence_numbers

    if claim_numbers and evidence_numbers and not shared_numbers:
        if len(claim_numbers) <= 2 and len(evidence_numbers) <= 2:
    ...
    shared_years = claim_years & evidence_years

    if claim_years and evidence_years and not shared_years:
        try:
            claim_year = max(int(y) for y in claim_years)
            evidence_year = max(int(y) for y in evidence_years)
            if abs(claim_year - evidence_year) >= 2:
    ...
    # NEGATION CONFLICTS
    # ==========================================
    claim_has_negation = bool(claim_struct["negations"])
    evidence_has_negation = bool(evidence_struct["negations"])
    ...
    # ==========================================
    contextual_conflicts = detect_contextual_conflicts(
        claim_struct, evidence_struct)
...
def compare_fact_structure(claim, evidence):
    ...
    for claim_action in claim_struct["actions"]:
        for evidence_action in evidence_struct["actions"]:
            similarity = fuzz.ratio(claim_action, evidence_action)
            if similarity >= 85:
                action_overlap += 1
    ...
    # EVENT TERM MATCHING
    # =================================================
    event_overlap = len(set(claim_struct["event_terms"]) & set(
        evidence_struct["event_terms"]))
    ...
    if action_overlap > 0 or event_overlap > 0 or entity_score > 3:
    ...
    # Properly nested debug blocks inside the function
    print("\n[DETECT CONFLICTS DEBUG]")
    print("CLAIM YEARS:", claim_struct["years"])
    print("EVIDENCE YEARS:", evidence_struct["years"])
    print("CLAIM NUMBERS:", claim_struct["numbers"])
    print("EVIDENCE NUMBERS:", evidence_struct["numbers"])
    ...
    if claim_struct["recency_terms"] and evidence_struct["years"]:
        try:
            latest_year = max(int(y) for y in evidence_struct["years"])
